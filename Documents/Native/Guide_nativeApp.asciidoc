...
```java
AMoAdNativeViewManager.getInstance(this).createView(... new AMoAdNativeFailureListener() {
  @Override
...
```java
AMoAdNativeViewManager.getInstance(this).renderAd(... new AMoAdNativeFailureListener() {
  @Override
  public void onFailed(String sid, String tag) {
    // 広告の取得失敗を検知
  }
